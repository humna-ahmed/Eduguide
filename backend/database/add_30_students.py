...
for i, name in enumerate(muslim_names[:30], start=1):
    ...
    semester = 1  # Same semester for all
    department = "CS"
...
# Get all student IDs
cur.execute("SELECT student_id, registration_no FROM students WHERE semester = 1 AND department = 'CS'")
...
# Count students
cur.execute("SELECT COUNT(*) FROM students WHERE semester = 1 AND department = 'CS'")
...
cur.execute("""
    ...
    FROM students s
    WHERE s.semester = 1 AND s.department = 'CS'
    LIMIT 5
""")
